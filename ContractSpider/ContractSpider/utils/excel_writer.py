...
def has_data_in_sheet(file_path: str, sheet_name: str = "Contracts") -> bool:
    try:
        wb = load_workbook(file_path, read_only=True)
        if sheet_name not in wb.sheetnames:
            return False
        ws = wb[sheet_name]
        for row in ws.iter_rows(min_row=2, max_row=2):  # 第一行一般是表头，从第二行查数据
            for cell in row:
